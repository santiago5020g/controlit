...
public function getSolicitudes_reporte_control_interno($sql_parameter)
{
	$this->Solicitudes();
	$obj=new Conn();
	$sql="select so.id as id,so.fecha_inicio_creacion as fecha_solicitud,so.contacto as solicitante,so.fecha_inicio_programado as fecha_inicio_programado,so.hora_inicio_programado as hora_inicio_programado,
	...
	inner join estado est on so.idestado=est.idestado where $sql_parameter
	group by so.id";
	$this->solicitudes_reporte=$obj->megaShot($sql);
	$obj="";
	return $this->solicitudes_reporte;
...
public function getTareas_reporte_control_interno($lista_solictudes,$sql_parameter)
{
	$this->Solicitudes();
	$obj=new Conn();
	...
	inner join tblsolicitudes so on so.id=ta.idso
     inner join tblsede se on se.id=ta.idsitio where ta.idso in($lista_solictudes) and $sql_parameter";
...
public function getTareas_reporte($lista_solictudes)
{
	...
	(ta.fecha_inicio, ta.hora_inicio ) ,Timestamp(ta.fecha_inicio,ta.hora_fin)) as tiempo_real,ca.descripcion as categoria,se.Sede as sede,ta.fecha_inicio as fecha_inicio,ta.hora_inicio as hora_inicio,ta.hora_fin as hora_fin,ta.observaciones as detalles from tbltareas ta inner join tblcategoria ca on ta.idcategoria=ca.categoriaid
	inner join sec_users usuarios  on usuarios.login=ta.propietario
     inner join tblsede se on se.id=ta.idsitio where ta.idso in($lista_solictudes)";
     $this->tareas_reporte=$obj->megaShot($sql);
     $obj="";
     return $this->tareas_reporte;
}
...
WHERE so.id
IN ($lista_solictudes) and tipos.id !=3 and tipos.id!=10
...
public function getTiposervicio_control_interno($lista_solictudes,$sql_parameter)
{
	$this->Solicitudes();
	$obj=new Conn();
	$sql="SELECT SUM( ta.tiempo_efectivo ) AS suma_tarea_tiempo_efetivo, tipos.nombre AS tipos_nombre, sum(TIMESTAMPDIFF (MINUTE, Timestamp
	(ta.fecha_inicio, ta.hora_inicio ) ,Timestamp(ta.fecha_inicio,ta.hora_fin))) as suma_tiempo_real
FROM tbltipo_servicio tipos
INNER JOIN tblcategoria ca ON ca.tipo_servicio = tipos.id
INNER JOIN tbltareas ta ON ta.idcategoria = ca.categoriaid
INNER JOIN tblsolicitudes so ON so.id = ta.idso
WHERE so.id
IN ($lista_solictudes) and tipos.id !=3 and tipos.id!=10 and $sql_parameter
GROUP BY tipos.nombre";
