...
$proyecto1 = $_REQUEST["proyecto"];
$proyecto = $obj_reportes->get_proyectos($proyecto1);
for($i=0;$i<sizeof($proyecto);$i++)
{
$pdf->SetXY(10,20);
$pdf->SetFont('Arial','b',9);
$pdf->Cell(20,10,"proyecto");
$pdf->SetFont('Arial','',9);
$pdf->Cell(20,10,$proyecto[$i][0]);

$pdf->SetFont('Arial','b',9);
$pdf->Cell(20,10,"Cliente");
$pdf->SetFont('Arial','',9);
$pdf->Cell(20,10,$proyecto[$i][3]);
$pdf->ln();

$pdf->SetFont('Arial','b',9);
$pdf->Cell(40,10,"Nombre del proyecto");
$pdf->SetFont('Arial','',9);
$pdf->Cell(20,10,utf8_decode($proyecto[$i][4]));
$pdf->ln();
$pdf->ln();

$so = $obj_reportes->getSolicitudes_reporte_proyecto($proyecto[$i][0]);
for($y=0;$y<sizeof($so);$y++)
{
	$solicitudes2 .= $so[$y]["id"];
	if($y==sizeof($so) -1) 	{ $solicitudes2 .=""; }
	else {$solicitudes2 .=",";}
...
$pdf->SetFont('Arial','b',9);
$pdf->Cell(15,10,utf8_decode("cliente"));
$pdf->SetFont('Arial','',9);
...
}//fin for($y=0;$y<sizeof($so);$y++)

if($i<sizeof($proyecto))
{$pdf->AddPage();}
}//fin for($i=0;$i<sizeof($proyecto);$i++
